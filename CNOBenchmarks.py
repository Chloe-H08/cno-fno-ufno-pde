...
#         self.reader = h5py.File(file_data_train, 'r')
#         # if 'min_u0' in self.reader and 'max_u0' in self.reader:
#         #     self.min_data = self.reader['min_u0'][()]
#         #     self.max_data = self.reader['max_u0'][()]
#         # else:
#         #     self.min_data = 0.0
#         #     self.max_data = 1.0  # or load this dynamically from the data
#         #     print("Warning: 'min_u0' and 'max_u0' not found in file, using defaults")
            
#         # if 'min_u' in self.reader and 'max_u' in self.reader:
#         #     self.min_model = self.reader['min_u'][()]
#         #     self.max_model = self.reader['max_u'][()]
#         # else:
#         #     print("Warning: 'min_model' not found in dataset. Using defaults.")
#         #     self.min_model = 0.0
#         #     self.max_model = 1.0
...
from neuraloperator.neuralop.data.datasets.darcy import load_darcy_pt

class Darcy:
    def __init__(self, network_properties, device, batch_size, training_samples=512, s=128, in_dist=True):
        if "in_size" in network_properties:
            self.in_size = network_properties["in_size"]
            assert self.in_size == 128, "Only 128x128 resolution supported here."
        else:
            raise ValueError("You must specify the computational grid size.")

        network_properties = default_param(network_properties)
        torch.manual_seed(network_properties["retrain"])

        self.model = CNO(
            in_dim=1 + 2 * network_properties["FourierF"],
            in_size=self.in_size,
            N_layers=network_properties["N_layers"],
            N_res=network_properties["N_res"],
            N_res_neck=network_properties["N_res_neck"],
            channel_multiplier=network_properties["channel_multiplier"],
            conv_kernel=network_properties["kernel_size"],
            cutoff_den=network_properties["cutoff_den"],
            filter_size=network_properties["filter_size"],
            lrelu_upsampling=network_properties["lrelu_upsampling"],
            half_width_mult=network_properties["half_width_mult"],
        ...
        # Use the built-in PT loader for 128x128
        data_root = "neuraloperator/neuralop/data/datasets/data"
        train_loader, test_loaders, _ = load_darcy_pt(
        ...
        # self.val_loader = test_loaders[128]  # For validation/testing
        self.train_loader = self._resize_loader(train_loader, target_size=128)
        self.val_loader = self._resize_loader(test_loaders[128], target_size=128)
        self.test_loader = self._resize_loader(test_loaders[128], target_size=128)

    def _resize_loader(self, loader, target_size):
        def _resize_batch(batch):
            x, y = batch['x'], batch['y']  # unpack from dict
            x = x.unsqueeze(1) if x.ndim == 3 else x  # [B, 1, X, T]
            y = y

            x_resized = F.interpolate(x, size=(target_size, target_size), mode='bilinear', align_corners=False)
            y_resized = F.interpolate(y, size=(target_size, target_size), mode='bilinear', align_corners=False)

            return x_resized.float(), y_resized.float()

        return [( _resize_batch(batch) ) for batch in loader]
...
class BurgersCNO:
    def __init__(self, network_properties, device, batch_size, training_samples=512):
        if "in_size" in network_properties:
            self.in_size = network_properties["in_size"]
        else:
            raise ValueError("You must specify the computational grid size.")
        ...
        data_path = "neuraloperator/neuralop/data/datasets/data"
        train_loader, test_loaders, _ = load_mini_burgers_1dtime(
            data_path=data_path,
            n_train=training_samples,
            n_test=200,
            batch_size=batch_size,
        ...
        # # Use directly — no permute needed
        # self.train_loader = train_loader
        # self.val_loader = test_loaders[16]
        

        self.train_loader = self._resize_loader(train_loader, target_size=128)
        self.val_loader = self._resize_loader(test_loaders[16], target_size=128)
        self.test_loader = self._resize_loader(test_loaders[16], target_size=128)
...
from neuraloperator.neuralop.data.datasets import load_navier_stokes_pt
