...
if __name__ == "__main__":
    ...
    sys.stdout = TeeLogger("logs/navier_stokes_fno_train_log.txt")
    ...
    # Limit training dataset to only 1000 samples
    train_dataset = train_loader.dataset
    train_loader = DataLoader(
        dataset=train_dataset,
        batch_size=config.data.batch_size,
        shuffle=not config.distributed.use_distributed,
        num_workers=4,
        pin_memory=True
    )
    ...
    if is_logger:
        model_path = os.path.join("TrainedModels", "navier_stokes_fno_final_model.pth")
